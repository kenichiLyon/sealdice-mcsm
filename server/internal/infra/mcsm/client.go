...
// Client handles communication with the MCSM API.
type Client struct {
	BaseURL string
	APIKey  string
	HTTP    *http.Client
...
// NewClient creates a new MCSM client.
...
func (c *Client) FindInstanceDaemonID(instanceID string) (string, error) {
	// 1. Get all daemons
...
func (c *Client) InstanceAction(instanceID, action string) error {
...
func (c *Client) ForceStopInstance(id string) error {
	return c.InstanceAction(id, "kill")
}

// GetInstanceStatus returns the status of an instance.
func (c *Client) GetInstanceStatus(id string) (interface{}, error) {
...
func (c *Client) GetDashboard() (interface{}, error) {
	// Based on docs, remote_services_system returns list of nodes with system info
	// Or we can try /api/dashboard if available, but let's stick to the one mentioned in reference 1
	// which returns node list with system info.
	// Reference 2 shows "api_dashboard.html" returning a single object with "system", "chart", "remote".
	// Let's try /api/dashboard as it seems to match the "Dashboard" requirement better.

	// Trying /api/dashboard first
	// Note: The user provided URL is just the doc page name.
	// Assuming /api/dashboard based on "api_dashboard.html" title "Dashboard API".
